...
begin
	...
	-------------------------------------------------------------------
	io_out <= "1000100000000000" when result_valid = '0'
	     else rx_data(11) & rx_data(11) & rx_data(11) & rx_data(11) & rx_data;
	--         ^ sign-extend bit 11 across the upper 4 bits
	...
	begin
		if nrst = '0' then
			...
			if IO_WRITE = '1' and IO_ADDR = "00011000000" then
				-- Pack the 6 config bits from SCOMP into the upper field.
				-- Lower 6 bits of tx_data are always 0 (don't-care).
				tx_data      <= IO_DATA(5 downto 0) & "000000";
				result_valid <= '0'; -- Invalidate stale result
			end if;
